...
import com.edu.msc.sliit.controller.BrailleApplicationRestController;
import com.edu.msc.sliit.model.ManuwalNoiseRemoval;
...
public class BraileeDetect {
    ...
    public HashMap<Integer, double[]> findRectangle() {
        ...
        for (MatOfPoint cnt : contours) {
            ...
            for (Point p : cnt.toList()) {
            	...
            	//if (!( (i == 188) || (i==1006)||(i==1507)||(i==1508)||(i==1509) ) ) {
            	//(!( (i == 331) || (i==332) ) )
            ...
            if(noiseDetect(i)) {
                if (Imgproc.contourArea(cnt) > 20) { //Eliminate Merge                    
                    ...
                    averageXYCordinates[1] = (maxY + minY) / 2;
                    capitalCities.put(i, averageXYCordinates);
                   // System.out.println("MatOfPoint no : " + i+" averageX : "+averageXYCordinates[0]+" averageY : "+averageXYCordinates[1]);
                }
            }
            x = 0;
            i++;
    ...
    //Noise Removal
    public boolean noiseDetect(int findcount) {   	 
    	return ManuwalNoiseRemoval.noisereport(BrailleApplicationRestController.image.width(),BrailleApplicationRestController.image.height(),findcount);
    }
}
